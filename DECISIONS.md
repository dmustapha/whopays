...
- Where: ARCHITECTURE.md §10, source-lock row 3.

## ADR-003: No member auth; owner uses a secret-to-session login
- Context: Convex Auth v2 is "super duper alpha"; members-by-email IS the thesis.
...
- Context: AI Gateway is paid-plan-only; AgentRouter key failed validation + provenance risk.
- Decision: direct fetch with `response_format: json_schema, strict: true`.
- Rejected: AI Gateway (spend decision, paid); AgentRouter (dead key, rubric row-3 provenance risk).
- Consequence: OpenAI outage degrades to `pending_parse`, never fabricated output.
- Where: ARCHITECTURE.md §9.
...
- Where: ARCHITECTURE.md §3 (parse.ts), §8 (routeInbound).
...
- Where: ARCHITECTURE.md §11 (App.tsx).

## ADR-007: Emails plaintext in private tables; privacy enforced at the projection
- Context: PRD drafted "hashed + encrypted" member emails; Convex mutations are deterministic (no reliable crypto), and the DB isn't publicly readable anyway.
...
- Consequence: `audit-pii.ts` is the enforcement test; NN-4 unchanged.
- Where: ARCHITECTURE.md §4 Key Decisions.
...
- Where: ARCHITECTURE.md §5 (recordPayment).

## ADR-009: Anchor plan = owner-enrolled real household bill with anonymous labels (AMEND-1)
- Context: friend consents unavailable; LAW 4 forbids fabricated members.
...
- Rejected: fake names (NN-1 violation); friend initials (needs consent); dropping the anchor surface (loses the no-hidden-clock pairing proof).
- Consequence: demo video's mechanism beats run on operator-controlled real inboxes.
- Where: WINNER-BRIEF AMEND-1; ARCHITECTURE.md §5 (enrollSeat), §6 (closeCycle skip).
...
- Where: ARCHITECTURE.md §20 step 6; source-lock row.
