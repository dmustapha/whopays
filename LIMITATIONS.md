...
- Crawled prices with provenance (source URL + checked-time) via Firecrawl `location:NG`; weekly re-check + owner "re-crawl now" button + change→recompute→notify; add-any-plan-by-URL with OpenAI extraction.
...
- Payment *verification* of any kind. Reason: core design principle — we reconcile and the owner confirms; bank-grade verification is not claimable or buildable here.
...
- Stranger-matching marketplace (Spliiit-class). Reason: scope boundary — WhoPays is the ledger for YOUR existing group.
